          ...
          {paginatedInvoices.map(inv => (
            <div 
              ...
              className={`p-3.5 space-y-2.5 transition-colors ${
                inv.isCancelled ? 'opacity-60 bg-red-50/30 dark:bg-red-950/20' : ''
              } ${selectedInvoiceIds.includes(inv.id) ? 'bg-red-50/40 dark:bg-red-950/30' : ''}`}
            >
              <div className="flex items-start justify-between gap-2">
                <div>
                  ...
                  <span className="text-[10px] text-slate-400 font-mono">
                    {inv.invoiceDate} • 📞 {inv.customerPhone}
                  </span>
                </div>
              ...
              {inv.petName && (
                <div className="flex items-center gap-1.5">
                  <span className="text-[10px] font-semibold text-[#D62828] bg-red-50 dark:bg-red-950/60 px-2 py-0.5 rounded-md">
                    🐾 {inv.petName}
                  </span>
                </div>
              )}
              ...
              <div className="flex items-center justify-end gap-1.5 pt-1">
                {hasPermission(currentUser, 'invoices_view') && (
                  <button
                    ...
                    className="px-2.5 py-1.5 min-h-[36px] bg-slate-100 dark:bg-zinc-800 text-slate-700 dark:text-zinc-200 rounded-lg text-xs font-bold flex items-center gap-1 cursor-pointer"
                  >
                ...
                {hasPermission(currentUser, 'invoices_download_pdf') && (
                  <button
                    ...
                    className="px-2.5 py-1.5 min-h-[36px] bg-blue-50 dark:bg-blue-950/60 text-blue-700 dark:text-blue-300 rounded-lg text-xs font-bold flex items-center gap-1 cursor-pointer"
                  >
