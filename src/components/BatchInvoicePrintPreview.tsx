...
import { Printer, ArrowLeft, X, Download } from 'lucide-react';
...
export const BatchInvoicePrintPreview: React.FC<BatchInvoicePrintPreviewProps> = ({
  ...
  const handlePrint = () => {
    ...
    document.title = `THOP_Batch_Invoices_${invoices.length}_Invoices`;
    window.print();
    setTimeout(() => {
        ...
        <div className="no-print bg-slate-900 text-white p-3 sm:p-4 flex items-center justify-between border-b border-slate-800 shrink-0 gap-4">
          ...
          <div className="flex items-center space-x-2">
            ...
            >
              ...
              <span>Print / Download Batch PDF ({invoices.length})</span>
            </button>
        ...
        <div className="invoice-print-area print-container p-6 sm:p-10 overflow-y-auto flex-1 bg-white text-slate-900 font-sans text-xs select-text">
          {invoices.map((invoice, idx) => (
            <div key={invoice.id || idx} className={idx < invoices.length - 1 ? 'page-break mb-12 pb-8 border-b-4 border-slate-900' : ''}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 bg-slate-50 p-3 rounded-lg border border-slate-200 my-4">
                <div>
                  ...
                  <tbody>
                    {invoice.items.map((item, itemIdx) => (
                      <tr key={item.id || itemIdx} className="border-b border-slate-200">
                        <td className="p-2 border-r border-slate-200 text-center font-mono">{itemIdx + 1}</td>
